...
import type { OrganizerDashboardData } from "../../lib/db/organizer-dashboard";
...
const SIDEBAR_GROUPS = [
  {
    label: "Workspace",
    items: ["Dashboard", "Content Calendar", "My Events", "Create Post"],
  },
  {
    label: "Growth",
    items: ["Analytics", "Ad Manager", "Hashtags"],
  },
  {
    label: "Community",
    items: ["Messages", "Comments", "Snippets"],
  },
  {
    label: "Account",
    items: ["Org Profile", "Team Access", "Settings"],
  },
];
...
export function OrganizerPortal({ dashboard }: { dashboard: OrganizerDashboardData }) {
  const { organizer, overview, salesSeries, recentEvents, hashtags, activity, snippets } = dashboard;

  return (
    <div className="rounded-[34px] border border-[var(--border-subtle)] bg-[var(--bg-elevated)] shadow-[0_30px_90px_rgba(4,12,8,0.16)]">
      <div className="grid min-h-[820px] xl:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="border-b border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 xl:border-b-0 xl:border-r">
          ...
          <div className="mt-6 rounded-[24px] border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[var(--brand)]/12 font-display text-sm font-bold text-[var(--brand)]">
                {organizer.initials}
              </div>
                ...
                <div className="mt-2 space-y-1">
                  {group.items.map((item) => {
                    ...
                    return (
                      <div
                        key={item}
                        className={`rounded-2xl px-3 py-2.5 text-[13px] font-medium transition-colors ${
                          isActive
          ...
          <div className="mt-6 rounded-[24px] border border-[var(--brand)]/20 bg-[var(--brand)]/10 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--brand)]">
              Pro Plan
            </p>
            <p className="mt-2 text-[13px] leading-6 text-[var(--text-secondary)]">
              Unlimited boosts, richer audience analytics, and faster support for featured drops.
            </p>
          </div>
        </aside>

        <section className="min-w-0">
          <div className="border-b border-[var(--border-subtle)] bg-[var(--bg-card)] px-5 py-5 md:px-7">
          ...
          <div className="grid gap-6 p-5 md:p-7 2xl:grid-cols-[minmax(0,1fr)_280px]">
            <div className="min-w-0 space-y-6">
              <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <MetricCard
                  delta={overview.ticketSalesDelta}
                  icon={<Ticket size={18} weight="fill" />}
                  label="Ticket Sales"
                  value={formatCompactNumber(overview.ticketSales)}
                />
                <MetricCard
                  delta={overview.followerDelta}
                  icon={<UsersThree size={18} weight="fill" />}
                  label="Followers"
                  value={formatCompactNumber(overview.followerCount)}
                />
                <MetricCard
                  delta={overview.eventViewsDelta}
                  icon={<Waves size={18} weight="fill" />}
                  label="Event Views"
                  value={formatCompactNumber(overview.eventViews)}
                />
                <MetricCard
                  delta={overview.revenueDelta}
                  icon={<TrendUp size={18} weight="fill" />}
                  label="Revenue"
                  value={formatCompactNumber(overview.revenue)}
                />
              </section>
                          ...
                          <div className="flex h-full w-full items-end">
                            <div
                              className={`w-full rounded-t-[18px] ${
                                index === 4 ? "bg-[var(--brand)]" : "bg-[var(--brand)]/35"
                              }`}
                              style={{ height: `${height}%` }}
                            />
                          </div>
              ...
              <section className="rounded-[30px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 shadow-[0_16px_44px_rgba(6,14,9,0.08)]">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Link className="inline-flex items-center gap-2 text-[13px] font-semibold text-[var(--brand)]" href="/organizer">
                    View all
            ...
            <div className="space-y-4">
              <article className="rounded-[28px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 shadow-[0_16px_44px_rgba(6,14,9,0.08)]">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {organizer.verified ? (
                    <CheckCircle size={18} className="text-[var(--brand)]" weight="fill" />
                  ) : null}
                </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {organizer.socialLinks.map((link) => (
                      <a
                        ...
                        className="inline-flex items-center gap-2 rounded-full border border-[var(--border-subtle)] px-3 py-1.5 text-[12px] font-medium text-[var(--text-secondary)] transition hover:text-[var(--text-primary)]"
                        href={link.href}
                        rel="noreferrer"
                        target="_blank"
                      >
                ...
                {organizer.websiteUrl ? (
                  <a
                    className="mt-5 inline-flex items-center gap-2 text-[13px] font-semibold text-[var(--brand)]"
              ...
              <article className="rounded-[28px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 shadow-[0_16px_44px_rgba(6,14,9,0.08)]">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  {[
                    ...
                    { href: "/organizer", label: "Boost Event", icon: <MegaphoneSimple size={18} /> },
                    { href: "/profile", label: "Edit Profile", icon: <NotePencil size={18} /> },
                  ].map((item) => (
                    <Link
                      key={item.label}
...
export function OrganizerUpgradeGate({ firstName }: { firstName: string }) {
  return (
    <section className="mx-auto max-w-[880px] rounded-[34px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-8 shadow-[0_24px_72px_rgba(6,14,9,0.12)] md:p-12">
