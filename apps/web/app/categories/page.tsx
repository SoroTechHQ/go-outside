    ...
    <main className="page-grid min-h-screen pb-24">
      <section className="container-shell py-10">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {categories.map((cat) => {
            const count = events.filter((e) => e.categorySlug === cat.slug).length;
            return (
              <Link key={cat.slug} href={`/categories/${cat.slug}`} className="block">
                <ShellCard className="h-full transition hover:border-[var(--neon)]">
                  ...
                  <h2 className="mt-4 font-display text-3xl italic text-[var(--text-primary)]">{cat.name}</h2>
                  <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">{cat.description}</p>
