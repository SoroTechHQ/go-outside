...
const CHANNEL_LIST_OPTIONS = { limit: 10, presence: false, state: true, watch: true } as const;
const REQUEST_LIST_OPTIONS = { limit: 20, presence: false, state: true, watch: true } as const;
...
function MessagesShell({
  ...
  const [requestCountLoaded, setRequestCountLoaded] = useState(false);
  ...
  useEffect(() => {
    if (!client) return;
    if (inbox !== "requests" && requestCountLoaded) return;
    let cancelled = false;
    const refresh = async () => {
      try {
        const channels = await client.queryChannels(
          ...
          {},
          { limit: 25, state: false, watch: false },
        );
        ...
        setRequestCountLoaded(true);
      } catch { /* ignore */ }
    };
    const timer = window.setTimeout(() => void refresh(), inbox === "requests" ? 0 : 2500);
  ...
  }, [client, identity.id, inbox, requestCountLoaded]);
