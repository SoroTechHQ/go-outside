    ...
    return (
      <main className="min-h-screen bg-[var(--bg-base)] pb-48 text-[var(--text-primary)]">
        <div className="mx-auto max-w-lg px-4 pt-8 text-center">
          <p className="text-[var(--text-secondary)]">Please sign in to view your tickets.</p>
        </div>
      </main>
    );
  }
  ...
  const upcomingEventMap = new Map(
    await Promise.all(
      upcoming.map(async (t) => {
        const ev = await getEventBySlug(t.eventSlug);
        return [t.id, ev] as const;
      })
    )
  );
        ...
        <section className="mt-9">
          ...
          {upcoming.length > 0 ? (
            <div className="no-scrollbar -mx-4 flex gap-3 overflow-x-auto px-4 pb-1">
              {upcoming.map((ticket) => {
                const event = upcomingEventMap.get(ticket.id);
