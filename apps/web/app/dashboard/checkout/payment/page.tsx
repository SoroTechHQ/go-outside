            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {(
                [
                  ...
                  { id: "bank_transfer", label: "Bank Transfer", icon: Lock },
                ] as const
                ...
                >
                  ...
                  <span
                    className={`text-[11px] font-semibold ${
                      method === id ? "text-[var(--brand)]" : "text-[var(--text-secondary)]"
                    }`}
                  >
                    {label}
                  </span>
            ...
            <AnimatePresence mode="wait">
              ...
              {method === "card" && (
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2.5 rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] px-3.5 py-2.5">
                      <CreditCard size={15} className="text-[var(--text-tertiary)] shrink-0" />
                      <input
                        className="flex-1 bg-transparent text-[14px] text-[var(--text-primary)] outline-none placeholder:text-[var(--text-tertiary)]"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        className="w-full rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] px-3.5 py-2.5 text-[14px] text-[var(--text-primary)] outline-none placeholder:text-[var(--text-tertiary)]"
                        placeholder="MM / YY"
                        type="text"
                      />
                    </div>
              ...
              {method === "bank_transfer" && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -8 }}
                  initial={{ opacity: 0, y: 8 }}
                  className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-4 space-y-2"
                >
                  <p className="text-[13px] font-semibold text-[var(--text-primary)]">Bank Transfer Details</p>
                  <div className="space-y-1 text-[13px] text-[var(--text-secondary)]">
                    <p>Bank: <span className="font-medium text-[var(--text-primary)]">GCB Bank</span></p>
                    <p>Account: <span className="font-medium text-[var(--text-primary)]">1234567890</span></p>
                    <p>Reference: <span className="font-medium text-[var(--text-primary)]">GO-{Date.now().toString().slice(-6)}</span></p>
                  </div>
                  <p className="text-[11px] text-[var(--text-tertiary)] mt-2">
                    Transfer your payment and use the reference above. Your tickets will be confirmed within 1–2 hours.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
