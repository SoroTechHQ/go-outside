...
export default async function OrganizerSnippetsPage() {
  ...
  const avgRating = snippets.length > 0
    ? (snippets.reduce((s, sn) => s + (sn.rating ?? 5), 0) / snippets.length).toFixed(1)
    : "—";
  ...
  return (
    <div>
      ...
      <div className="relative overflow-hidden border-b border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 md:p-7">
        ...
        <h1 className="relative mt-1 text-[1.5rem] font-bold tracking-tight text-[var(--text-primary)]">Snippets</h1>
      ...
      <div className="p-5 md:p-7">
        {snippets.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-[22px] border border-[var(--border-subtle)] bg-[var(--bg-card)] px-6 py-20 text-center shadow-[0_2px_12px_rgba(5,12,8,0.05)]">
            ...
            <p className="mt-4 text-[15px] font-semibold text-[var(--text-primary)]">No snippets yet</p>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                { label: "Total snippets",   value: snippets.length.toString(),                                    accent: accents[0]! },
                { label: "Avg rating",        value: avgRating,                                                    accent: accents[1]! },
                { label: "5-star reviews",    value: snippets.filter((s) => (s.rating ?? 5) === 5).length.toString(), accent: accents[2]! },
                { label: "Events covered",    value: new Set(snippets.map((s) => s.event_id)).size.toString(),     accent: accents[3]! },
              ].map((k) => (
            ...
            <div className="space-y-2.5">
              {snippets.map((snippet) => {
                ...
                return (
                  <div key={snippet.id} className="rounded-[16px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4 transition hover:border-[var(--brand)]/20 hover:shadow-[0_4px_16px_rgba(5,12,8,0.06)]">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2.5">
                            ...
                            <StarRow rating={snippet.rating ?? 5} />
                          </div>
                          <p className="truncate text-[11px] text-[var(--brand)] max-w-[180px]">{eventTitle}</p>
                        </div>
                        <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-secondary)]">{snippet.body}</p>
                      </div>
                    </div>
                  </div>
                );
