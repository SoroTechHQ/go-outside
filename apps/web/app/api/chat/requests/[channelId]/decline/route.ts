...
export async function POST(
  ...
  try {
    ...
    if (data.go_initiated_by === clerk.id) {
      return jsonError(403, "Cannot decline your own request");
    }

    // Mark declined, then hide for the recipient — sender is not notified
    await channel.updatePartial({ set: { go_channel_state: "declined" } as Record<string, unknown> });
    await channel.hide(clerk.id);

    return jsonNoStore({ success: true });
  } catch (err) {
    console.error("[POST /api/chat/requests/decline]", err);
    return jsonError(500, "Could not decline request");
  }
}
